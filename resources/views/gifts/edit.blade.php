    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Edit Gift Idea for {{ $contact->name }}
            </h2>
    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('gifts.update', [$familyGroup, $contact, $gift]) }}">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="flex gap-4">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="is_public" value="1"
                                       {{ old('is_public', $gift->is_public) == '1' ? 'checked' : '' }}
                            ...
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="is_public" value="0"
                                       {{ old('is_public', $gift->is_public) == '0' ? 'checked' : '' }}
                                       class="text-indigo-600 focus:ring-indigo-500">
                    ...
                    <div class="mb-6 p-4 bg-red-50 border border-red-100 rounded-lg">
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="on_christmas_list" value="1"
                                   {{ old('on_christmas_list', $gift->on_christmas_list) ? 'checked' : '' }}
